...
help:
	...
	@echo "  make install          List one-time setup commands"
	@echo "  make install-deps     Install host tooling (tilt, docker, openjdk, node) via Homebrew"
	@echo "  make install-companion  cd companion && npm install + playwright install chromium"
	@echo "  make install-hooks    Point this clone's git hooks at .githooks/"
	@echo "  make install-all      Run install-deps + install-companion + install-hooks"
...
# Tilt orchestrates the dev stack: postgres in Docker (reused via pois-up),
# backend + companion on the host with file-watch + auto-restart. Tilt UI
# is at http://localhost:10350.
tilt-up:
	tilt up
...
# `make install` is informational — it echoes the install steps so you can
# run them piecewise. `make install-all` runs them in sequence.
install:
	@echo 'One-time local-dev setup. Run "make install-all" to do everything,'
	@echo 'or run these individually:'
	@echo ''
	@echo '  make install-deps        # tilt, docker, openjdk, node (via Homebrew)'
	@echo '  make install-companion   # cd companion && npm install + playwright chromium'
