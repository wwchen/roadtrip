...
private data class CachedStatus(val recgovReachable: Boolean, val checkedAt: OffsetDateTime)
...
fun Route.statusRoutes() {
    get("/api/campsite/status") {
        ...
        val cur = cache.get()
        val fresh = cur != null && java.time.Duration.between(cur.checkedAt, now).toMillis() < CACHE_MS
        val recgovReachable = if (fresh) cur!!.recgovReachable else {
            val ok = runCatching {
                log.info("Status: checking rec.gov reachability")
                httpClient.get("https://www.recreation.gov/") {
