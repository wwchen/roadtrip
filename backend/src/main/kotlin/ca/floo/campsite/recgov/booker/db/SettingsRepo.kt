...
class SettingsRepo(private val ctx: DSLContext) {
    fun get(key: String): String? =
        ctx.select(SETTINGS.VALUE).from(SETTINGS).where(SETTINGS.KEY.eq(key)).fetchOne()?.value1()

    fun all(): Map<String, String> =
        ctx.selectFrom(SETTINGS).fetchMap(SETTINGS.KEY, SETTINGS.VALUE)
            .filterKeys { it != null }
            .mapKeys { it.key!! }
            .mapValues { it.value ?: "" }

    fun set(key: String, value: String) {
        ctx.insertInto(SETTINGS, SETTINGS.KEY, SETTINGS.VALUE)
            .values(key, value)
    ...
    fun setMany(updates: Map<String, String>) {
        ctx.transaction { trx ->
            val tx = trx.dsl()
            for ((k, v) in updates) {
                tx.insertInto(SETTINGS, SETTINGS.KEY, SETTINGS.VALUE)
                    .values(k, v)
                    .onConflict(SETTINGS.KEY)
                    .doUpdate()
