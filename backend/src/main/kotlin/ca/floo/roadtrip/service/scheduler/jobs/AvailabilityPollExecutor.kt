        ...
        val windowFor: (
            ...
            targetWindow?.let {
                ...
                ca.floo.roadtrip.models.availability
                    .AvailabilityWindows(target = it, fetch = fetchWindow)
            }
        }

        // Vendor governor: acquire the token cost for every group the batcher
        // will actually FETCH, BEFORE any upstream call. Groups whose polling
        // window is null (all target dates elapsed) are skipped by the batcher
        // — no upstream call — so they are excluded from the count; charging a
        // token for a non-fetch would waste it and could starve a bucket,
        // delaying retirement of an all-elapsed poller. On starvation, skip
        // the fetch entirely — no upstream call, no wasted 429, and no run row
        // (a starved tick is a non-event, like an empty window, not a failure,
        // so it never feeds consecutive-failure backoff). Reschedule soon so
        // the poller retries once the bucket refills. Per-poller backoff stays
        // the reactive net for real upstream failures.
        val bucketTokens = batcher.countFetchTokens(resolved, windowFor)
        if (bucketTokens > 0 && !limiter.tryAcquire(poller.provider, bucketTokens)) {
            log.info(
