...
class AvailabilityRepoTest : SharedDbTest() {
    ...
    fun `unchanged status bumps last_observed_at in place, no new row`() {
        val reservableId = seedReservable("100")
        val repo = AvailabilityRepo(ctx)
        val t1 = Instant.parse("2026-06-18T10:00:00Z")
        val t2 = Instant.parse("2026-06-18T10:05:00Z")
        repo.recordObservations(runId = null, listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.RESERVED, t1)))
        val transitions =
            repo.recordObservations(
                runId = null,
                listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.RESERVED, t2)),
            )
        ...
        val current = repo.readCurrent(listOf(reservableId), listOf(date)).single()
    ...
    fun `status change inserts a new row linked by previous_id`() {
        ...
        val t1 = Instant.parse("2026-06-18T10:00:00Z")
        val t2 = Instant.parse("2026-06-18T10:05:00Z")
        repo.recordObservations(null, listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.RESERVED, t1)))
        val transitions =
            repo.recordObservations(
                null,
                listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.AVAILABLE, t2)),
            )
    ...
    fun `markElapsedAsPast adds a past status-run for elapsed cells only`() {
        ...
        repo.recordObservations(
            null,
            listOf(
                AvailabilityRepo.Observation(reservableId, past, AvailabilityStatus.RESERVED, t),
                AvailabilityRepo.Observation(reservableId, future, AvailabilityStatus.RESERVED, t),
            ),
        )
        val inserted = repo.markElapsedAsPast(listOf(reservableId), today = LocalDate.parse("2026-07-04"))
        assertEquals(1, inserted)
        assertEquals(AvailabilityStatus.PAST, repo.readCurrent(listOf(reservableId), listOf(past)).single().status)
        assertEquals(AvailabilityStatus.RESERVED, repo.readCurrent(listOf(reservableId), listOf(future)).single().status)
    }
    ...
    fun `markElapsedAsPast is idempotent - an already-past cell is not re-marked`() {
        ...
        val past = LocalDate.parse("2026-06-01")
        val t = Instant.parse("2026-06-18T10:00:00Z")
        repo.recordObservations(null, listOf(AvailabilityRepo.Observation(reservableId, past, AvailabilityStatus.RESERVED, t)))
        val today = LocalDate.parse("2026-07-04")
        assertEquals(1, repo.markElapsedAsPast(listOf(reservableId), today = today))
        // Second run must not insert a duplicate past-run (would violate the previous_id chain).
        assertEquals(0, repo.markElapsedAsPast(listOf(reservableId), today = today))
        assertEquals(2, ctx.fetchCount(ctx.selectFrom(ca.floo.roadtrip.db.generated.tables.Availability.AVAILABILITY)))
    }
    ...
    fun `history walks the previous_id chain, observedFrom derives from previous`() {
        ...
        repo.recordObservations(null, listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.RESERVED, t1)))
        repo.recordObservations(null, listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.AVAILABLE, t2)))
        val runs = repo.listForReservable(reservableId)
    ...
    fun `summarize reports an open window from an available run`() {
        ...
        repo.recordObservations(null, listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.RESERVED, t0)))
        repo.recordObservations(null, listOf(AvailabilityRepo.Observation(reservableId, date, AvailabilityStatus.AVAILABLE, t1)))
        val stats = repo.summarize(reservableId, listOf(date), now = OffsetDateTime.parse("2026-06-18T12:00:00Z"))
        val s = stats.single()
    ...
    fun `summarize keeps a cell's current state even when its last observation predates the window`() {
        ...
        repo.recordObservations(null, listOf(AvailabilityRepo.Observation(reservableId, future, AvailabilityStatus.AVAILABLE, longAgo)))
        val now = OffsetDateTime.parse("2026-07-05T00:00:00Z")
        val s = repo.summarize(reservableId, listOf(future), now = now, windowHours = 24 * 7).single()
