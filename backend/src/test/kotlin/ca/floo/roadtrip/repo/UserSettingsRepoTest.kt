...
class UserSettingsRepoTest : SharedDbTest() {
    ...
    @Test fun `saveRecgovCredentials stores username and sealed password`() {
        ...
        repo.saveRecgovCredentials(u, username = "ada@example.com", passwordCipher = cipher, passwordHint = "cdef")
        val s = repo.find(u)!!
        assertEquals("ada@example.com", s.recgovUsername)
        assertContentEquals(cipher, s.recgovPasswordCipher)
        assertEquals("cdef", s.recgovPasswordHint)
    }

    @Test fun `saveRecgovCredentials with null password preserves the stored one`() {
        val u = newUser()
        val cipher = byteArrayOf(0x7, 0x8)
        repo.saveRecgovCredentials(u, "ada@example.com", cipher, "cdef")
        repo.saveRecgovCredentials(u, "grace@example.com", passwordCipher = null, passwordHint = null)
        val s = repo.find(u)!!
    ...
    @Test fun `clearRecgov nulls all three columns but keeps the Slack token`() {
        val u = newUser()
        repo.setSlackToken(u, byteArrayOf(9), "beef")
        repo.saveRecgovCredentials(u, "ada@example.com", byteArrayOf(1), "cdef")
        repo.clearRecgov(u)
        val s = repo.find(u)!!
        assertNull(s.recgovUsername)
        assertNull(s.recgovPasswordCipher)
        assertNull(s.recgovPasswordHint)
