...
class CachePolicyTest {
    ...
    fun `mutable API responses are not browser cached`() {
        assertEquals(
            "no-store",
            cacheOptionsFor("/api/watches", ContentType.Application.Json)?.cacheControl.toString(),
        )
        assertEquals(
            "no-store",
    ...
    fun `static assets keep deploy-friendly cache policy`() {
        assertEquals(
            "no-cache",
            cacheOptionsFor("/web/sandbox-banner.js", ContentType.Text.JavaScript)?.cacheControl.toString(),
        )
        assertEquals(
            "max-age=86400",
