...
// legacy `web/design-system/*` primitives. It is an ordinary registry dependency
// (`@lew-ds/lds-react`), installed from npm.
//
// Every migrated page imports its primitives from `@ui` rather than reaching for
// `@lew-ds/lds-react` directly. That indirection has now paid for itself once:
// LDS was vendored under `frontend/vendor/` until 1.0.0 was published, and moving
// to the registry — including the `@lew/` → `@lew-ds/` scope rename — was a change
// to this file, `SeededTextField`, `styles.css` and `package.json`. No call site
// moved.
//
