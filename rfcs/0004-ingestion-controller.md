...
| 9   | 2026-09-11 | Terminal sinks bind to `Repos` from `JooqUnitOfWork.autocommit`; the import stays non-transactional.                             | Decision 6's future work is still future work. Binding sinks to an explicit autocommit bundle makes "this import is not atomic" visible at the wiring site instead of implied by a shared `DSLContext`, and localises the change to the binding site: step 6 rebinds the definitions inside `UnitOfWork.run` instead of once at construction. |
