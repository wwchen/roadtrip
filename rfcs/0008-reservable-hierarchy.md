...
**Reservables table backfill** at deploy time:

1. Read every existing campground POI's `provider_ref`.
2. For each rec.gov campground: fetch the upstream once, walk
   `campsites`, insert one `reservables` row per campsite. Link to the
   campground POI via `reservable_pois`.
3. For each Aspira map: fetch once, walk `resourceAvailabilities` keys,
   insert one `reservables` row per resource. Names + types come from
   the existing `aspira-leaves-{tenant}.json` ETL output.
4. Camis: skipped (stub adapter; no real reservables yet).

Backfill is one-shot. Subsequent updates happen via the existing ETL
schedule — adapters get a hook to upsert reservables alongside POI
upserts.
...
| PR | Scope | Risk |
|---|---|---|
| 1 | Schema: `reservables` + `reservable_pois` tables + jOOQ regen + repo | Low — additive |
| 2 | Backfill ETL: walk existing POIs, populate reservables + links | Medium — first time we touch upstream at scale outside the existing pipeline |
| 3 | BookingProvider port becomes 2D registry; adapters move to `<vendor>/site/` subdirs; no behavior change | Low — refactor; tests gate it |
| 4 | New routes (`/api/poi/{id}/...`, `/api/reservable/{rid}/...`); old routes deleted; FE migrates in same PR | Medium — atomic API rename, FE+BE in lockstep |
| 5 | Alerts migration: add `reservable_id` column; backfill at deploy; poller switches to per-reservable evaluation | Medium — touches the alert/poll code path |

PRs 1-3 are forward-compatible no-ops to the running system. PR 4 is the
big one. PR 5 closes the loop on the alert UI.
