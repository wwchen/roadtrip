...
class SandboxWorkflowTest(unittest.TestCase):
    def test_active_sandboxes_update_after_ci_completes(self) -> None:
        ...
        self.assertIn("Sandboxes are limited to branches in this repository", resolver)
    ...
    def test_automatic_updates_expire_with_the_host_reaper(self) -> None:
        ...
        self.assertIn("activatedAt || new Date().toISOString()", script)
    ...
    def test_in_flight_updates_stand_down_when_stopped(self) -> None:
        ...
        init = step("Init status comment")["with"]["script"]
        automatic_guard = init.index("process.env.AUTOMATIC === 'true'")
        first_update = init.index("github.rest.issues.updateComment")
        self.assertLess(automatic_guard, first_update)
        self.assertIn("core.setOutput('comment_id', String(existing.id))", init)
    ...
    def test_redeploy_targets_the_stable_pr_sandbox(self) -> None:
        ...
        self.assertIn("scripts/deploy.sh sandbox-up \"$pr_number\"", host_step["run"])
        self.assertIn("steps.pr.outputs.action == 'up'", host_step["if"])
        self.assertIn("ROADTRIP_DATA_SHA=\"$data_sha\"", host_step["run"])

    def test_redeploy_uses_the_immutable_release_path(self) -> None:
        install = step("Install runtime release")
        self.assertIn("steps.recheck.outputs.proceed == 'true'", install["if"])
        self.assertEqual("${{ steps.pr.outputs.head_sha }}", install["with"]["sha"])

        stop = step("Run on host (stop)")["run"]
        self.assertIn("$HOME/.roadtrip/current", stop)
        self.assertIn("scripts/deploy.sh sandbox-down", stop)
